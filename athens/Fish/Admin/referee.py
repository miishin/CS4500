...
GameResult: {"winners":[...], "Losers":[...], "Cheaters":[...]}
...
Players who do not respond within a specified timeout will be kicked. 
"""

# Custom error for timeouts
def player_interaction_timeout_handler(signum, frame):
	raise OSError("Player took too long to respond")

def try_fn_with_timeout(fn, duration, *args):
	"""
	Runs a given function with the given arguments but times out
	if the function takes longer than a given duration to run.
	:param fn: The function to run
	:param duration: The time limit (in seconds)
	:param args: The arguments for the function
	:return:
	"""
	signal.signal(signal.SIGALRM, player_interaction_timeout_handler)
...
class Referee:
			...
			for row in range(amtRow):
				...
				for col in range(amtCol):
					if amtHoles < maxHoles:
						# Can still make more holes
						fishInTile = random.randint(0,5)
					else:
						# Cannot add more holes
	...
	def placementPhase(self):
		"""
		Iteratively asks each player in order of youngest age for a position (row, column) to place a penguin at,
		...
		for i in range(0, rounds):
			for player in self.players:
				devtools.DevTools.visualizeState(self.state)
				placement = player.placeAvatar(self.state)
				if placement is None:
					self.kickPlayer(self.state.turn)
				elif self.checkValidPlacement(placement):
	...
	def runMovementRound(self):
		...
		for player in self.players:
			if not self.state.gameOver():
				movement = try_fn_with_timeout(player.moveAvatar, 60, self.state)
				if not movement:
					self.state.nextTurn()
	...
	def kickPlayer(self, playerIndex):
		...
		self.state.deletePlayer(playerIndex)
