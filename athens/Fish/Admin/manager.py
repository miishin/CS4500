...
class Manager(Manager_Interface):
    ...
    A Manager keeps track of the results so far. The results are a list of RoundResult.
    RoundResult is a list of GameResult ie. the results of each game in that round.
    The round number if specified by the index in self.results
    GameResult is as defined in the Referee class.
    """
    WINNERS = "winners"
    LOSERS = "losers"
    CHEATERS = "cheaters"
    queryKeys = [WINNERS, LOSERS, CHEATERS]
    TYPE = "type"
    BODY = "body"
    ...
    def informPlayersOfEnd(self):
        ...
        losers = self.getNonAdvancingPlayers()
        losers_message = {self.TYPE: TournamentUpdateType.TournamentLoss, self.BODY: "You have been eliminated"}
        loser_acks = self.notifyPlayers(losers, losers_message)
        return (winner_acks, loser_acks)
    ...
    def queryResults(self, key, round_number=-1):
        ...
        :param key: String. One of "winner", "loser", or "cheater" to get the results for
        :param round_number: the round number to get the results of. defaults to the last round
        :return: The requested results of the requested round as a list of player id
        """
        if key not in self.queryKeys:
    ...
    def allocatePlayers(self, players):
        ...
        :return: List of List of Player
        """
        player_groupings = []
        for idx in range(len(players)):
            if idx % self.MAXIMAL_NUM_PLAYERS == 0:
                player_groupings.append([])
            player_groupings[-1].append(players[idx])
            player_groupings[-1].sort(key=Manager._sort_player_key)
        if len(player_groupings[-1]) == 1:  # Check the case where there is a game of 1 player, split into 3 - 2
            player_groupings[-1].append(player_groupings[-2].pop(3))
            player_groupings[-1].sort(key=Manager._sort_player_key)
        return player_groupings

    # Method for sorting a list of players by age.
    @staticmethod
    ...
    def createGames(self, playerGroupings):
        ...
        for grouping in playerGroupings:
            self.currentRoundReferees.append(
                Referee(grouping, self.default_row_size, self.default_column_size, self.default_num_fish,
                        randomize=self.randomize, randomSeed=self.seed))
        self.results.append([])
    ...
    def checkIsTournamentOver(self):
        ...
        if len(current_players) <= 1:
            ...
            self.runRound()
            return True
        elif len(self.results) > 1 and set(self.queryResults(self.WINNERS, -2)) == set(
                self.queryResults(self.WINNERS, -1)):
            return True
        else:
            return False
    ...
    def runRound(self):
        ...
        for ref in self.currentRoundReferees:
            self.results[-1].append(ref.play())
    ...
    ### Misc. Functions used for Milestone 10 ###

    def getNumCheaters(self):
        """
        Returns the number of cheaters in the tournament
        :return: Number of players who cheated
        """
        numCheaters = 0
        for round in self.results:
            for game in round:
                numCheaters += len(game["cheaters"])
        return numCheaters

    def _setRowSize(self, n):
        """
        Set the row size to use for games
        :param n: the row size to set to
        :return: None
        """
        self.default_row_size = n

    def _setColSize(self, n):
        """
        Set the row size to use for games
        :param n: the column size to set to
        :return: None
        """
        self.default_column_size = n

    def _setNumFish(self, n):
        """
        Set the number of fish to use for games
        :param n: the number of fish to set to
        :return: None
        """
        self.default_num_fish = n

    def _setRandomize(self, b):
        """
        Sets the randomization of the board
        :param b: whether to randomize
        :return: None
        """
        self.randomize = b
